...
fn read_excel(file_path: &str, sheet_name: &str) -> Result<RecordBatch, Box<dyn Error>> {
    ...
    // Separate the header row to use as column names
    let header_row = range.rows().next().ok_or("Empty sheet or unable to read header")?;
    
    // Loop over each column in the header row to create field names
    for col_idx in 0..header_row.len() {
        ...
        // Iterate over the data rows only (skipping the header row)
